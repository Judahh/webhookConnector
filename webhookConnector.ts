...
import { PersistencePromise } from 'flexiblepersistence/persistenceAdapter/persistencePromise';
...
export class WebhookConnector {
  ...
  constructor(packageJSON: any, filename: string, link?: string, writeDB?: PersistenceAdapter, readDB?: PersistenceAdapter) {
    console.log('The Read is a singleton class and cannot be created!');
    this.handler = new Handler(writeDB, readDB);
   ...
   * GET all Heroes.
   */
  public startNgrok() {// TODO: change to serveo
    let arch = os.arch();
  ...
  private getWebhooks = () => {
    this.handler.readArray('webhooks', {}).then(this.webhooksReceived);
  }

  private webhooksReceived = (persistencePromise: PersistencePromise) => {
    if (persistencePromise.receivedItem.length > 0) {
      console.log(persistencePromise.receivedItem[0]);
      this.webhook.setId(persistencePromise.receivedItem[0].id);
      this.webhook.setLink(persistencePromise.receivedItem[0].link);
    } else {
      this.getNgrok();
    }
  }
